...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(isset($_POST['delete'])){ $pdo->prepare('DELETE FROM promotions WHERE id=?')->execute([(int)$_POST['delete']]); }
    else {
        // naive insert for demo; adjust columns per table
    }
}
$rows=$pdo->query('SELECT * FROM promotions ORDER BY id DESC')->fetchAll();
?>
<!doctype html><html><head><meta charset="utf-8"><title>Admin - Khuyến mãi</title>
